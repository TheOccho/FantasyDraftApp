...
<tr class="{% if(typeof player === "undefined") { %}empty{% } %}" data-pid="{% if(typeof player !== "undefined") { %}{%= player.getPlayerID() %}{% } %}">
	...
	<td style="text-align:left;" width="124px">{% if(typeof player !== "undefined") { %}{%= player.getFirstName().charAt(0) + ". " + player.getLastName() %}<br/><span class="player-team-pos">{%= player.getTeamAbbrev().toUpperCase() %} - {%= player.getQualifiedPositions().join(", ") %}{% } %}</td>
