		...
		<h3>Fantasy Notes:</h3>
			{% if(news.length > 0) { %}
				{% for(var i=0,l=news.length;i<l;i++) { %}
					<div><b>{%= news[i].display_ts %}</b> - {%= news[i].spin %}</div>
					{% if(i!==l-1) { %}</br>{% } %}
				{% } %}
			{% } else { %}
		...
		<h3 style="margin-top:10px;">Injury Updates:</h3>
			{% if(injury.length > 0) { %}
				{% for(var i=0,l=injury.length;i<l;i++) { %}
					<div><b>{%= injury[i].display_ts %}</b> - Injury Status: {%= injury[i].injury_status %}. {%= injury[i].injury_update %} Due Back: {%= injury[i].due_back %}.</div>
					{% if(i!==l-1) { %}</br>{% } %}
				{% } %}
			{% } else { %}
